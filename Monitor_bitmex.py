...
class Trader_bitmex:
    def __init__(self, auth_info, test=False):
        self.client = bitmex.bitmex(api_key=auth_info['api_public_key'], api_secret=auth_info['api_secret_key'],
                                    test=test)

    def make_order(self, orders):
        result = []
        for order in orders:
            result.append(self.client.Order.Order_new(symbol=order.symbol, orderQty=order.quantity,
